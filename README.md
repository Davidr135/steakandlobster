...
- `app.js` — report handling, installation/country geocoding, heat map rendering, chart rendering, and local storage logic

## Notes

- Reports are stored only in the browser's `localStorage` in the current implementation.
- Installation and country inputs are converted to map coordinates through the open Nominatim geocoding API at submission time.
